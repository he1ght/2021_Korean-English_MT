...
from model import str2dec, str2enc

# from copy_generator import Embeddings, CopyGenerator
from copy_generator import CopyGenerator
from embeddings import Embeddings
from utils.util_class import Cast
...
def build_task_specific_model(model_opt, fields):
    ...
    if model_opt.model_task == ModelTask.SEQ2SEQ:
        ...
        return model.NMTModel(encoder=encoder, decoder=decoder)
    else:
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if not model_opt.copy_attn:
        if model_opt.generator_function == "sparsemax":
            gen_func = modules.sparse_activations.LogSparsemax(dim=-1)
        else:
