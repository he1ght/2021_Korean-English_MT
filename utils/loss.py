...
from modules.sparse_activations import LogSparsemax
...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    """
    device = torch.device("cuda" if utils.misc.use_gpu(opt) else "cpu")
    ...
    if opt.copy_attn:
        criterion = modules.CopyGeneratorLoss(
    ...
    if opt.copy_attn:
        if opt.model_task == ModelTask.SEQ2SEQ:
            ...
            compute = modules.CopyGeneratorLMLossCompute(
                criterion, loss_gen, tgt_field.vocab,
                opt.copy_loss_by_seqlength,
                lambda_coverage=opt.lambda_coverage
            )
...
class LossComputeBase(nn.Module):
    ...
    def _stats(self, loss, scores, target):
        ...
        return utils.Statistics(loss.item(), num_non_padding, num_correct)
...
class CommonLossCompute(LossComputeBase):
    ...
    def _add_align_shard_state(self, shard_state, batch, range_start,
        ...
        ref_align = utils.make_batch_align_matrix(
