...
from inputters.inputter import IterOnDevice
...
def build_trainer(opt, device_id, model, fields, optim, model_saver=None):
    ...
    train_loss = utils.loss.build_loss_compute(model, tgt_field, opt)
    ...
    earlystopper = utils.EarlyStopping(
        opt.early_stopping, scorers=utils.scorers_from_opts(opt)) \
        if opt.early_stopping > 0 else None

    report_manager = utils.build_report_manager(opt, gpu_rank)
